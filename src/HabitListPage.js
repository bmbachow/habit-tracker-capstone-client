...
export default class HabitListPage extends React.Component {
	...
	completeHabit(event) {
		...
		console.log(data);
		let updatedTimesCompleted = parseInt(times_completed);
		console.log(updatedTimesCompleted);
		const payload = {
			habitId,
			user_id,
			name,
			notes,
			times_completed: updatedTimesCompleted + 1,
		};
		console.log(payload);
	...
	render() {
					...
					<div className='form-container'>
						<form className='habitUpdateForm' onSubmit={this.completeHabit}>
							<input
								type='hidden'
								name='habitId'
								defaultValue={habit.id}></input>
							<input
								type='hidden'
								name='user_id'
								defaultValue={habit.user_id}></input>
							<input
								type='hidden'
								name='name'
								defaultValue={habit.name}></input>
							<input
								type='hidden'
								name='notes'
								defaultValue={habit.notes}></input>
							<input
								type='hidden'
								name='times_completed'
								defaultValue={habit.times_completed}></input>
							<button type='submit' className='habitDeleteBtn'>
								Completed
							</button>
						</form>
		...
		return (
			<div className='outer-container'>
				<div className='inner-container'>{showHabit}</div>
			</div>
		);
	}
}
